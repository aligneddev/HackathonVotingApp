...
presentations.MapGet("/", async (AppDbContext db) =>
    await db.Presentations.Select(p => p.ToResponse()).ToListAsync());

presentations.MapPost("/", async (CreatePresentationRequest request, AppDbContext db) =>
{
...
presentations.MapGet("/{id:guid}", async (Guid id, AppDbContext db) =>
{
...
presentations.MapPut("/{id:guid}", async (Guid id, UpdatePresentationRequest request, AppDbContext db) =>
{
    var presentation = await db.Presentations.FindAsync(id);
    if (presentation is null) return Results.NotFound();
...
presentations.MapDelete("/{id:guid}", async (Guid id, AppDbContext db) =>
{
    var presentation = await db.Presentations.FindAsync(id);
    if (presentation is null) return Results.NotFound();
